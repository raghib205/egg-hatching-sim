...
function updateUI() {
  ...
  if (state.eggs.length < state.hatchCapacity) {
    for (const [type, egg] of Object.entries(EGGS)) {
      ...
      let tooltipText = `${type.charAt(0).toUpperCase() + type.slice(1)} Egg\n\nPossible pets:\n`;
      ...
      eggsContainer.appendChild(newEggButton);
    }
  }
...
window.addEventListener('scroll', () => {
  if (window.scrollY > 500 && !hasTransitioned) {
    ...
    document.documentElement.style..egg {
        background: linear-gradient(135deg, var(--container-bg) 0%, var(--bg-color) 100%);
      }
setProperty('--bg-gradient', defaultGradient);
  }
});
